...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/forks-ecosystem/forks-life-node/main/install.sh)
```
...
Чтобы веб-интерфейс мог управлять сервисами (перезапуск, включение), PHP-скриптам нужно дать права на выполнение команд `systemctl`.
...
Вставьте в открывшийся файл следующий код. Замените www-data на пользователя, от которого работает ваш веб-сервер:
...
- Используйте **SSL (HTTPS)** — напр��мер, с [Let's Encrypt (Certbot)](https://certbot.eff.org/).  
